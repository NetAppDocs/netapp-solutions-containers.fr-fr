...
NOTE: Il est possible que les pods Trident ne soient pas créés à ce stade. Si vous décrivez l’objet ReplicaSet, il se peut que vous voyiez une erreur concernant la violation de PodSecurity comme indiqué ci-dessous. Cette erreur se produit car l’espace de noms  `trident` créé lors de l’installation applique la norme de sécurité des pods restreinte (PSS) de Kubernetes. L’opérateur Trident requiert des privilèges système élevés pour gérer le stockage hôte et ne peut pas fonctionner sous les contraintes strictes d’un profil restreint. Pour corriger cela, accordez au namespace  `trident` un profil privilégié afin que le contrôleur ReplicaSet puisse lancer correctement les pods.
...
----
Appliquez les étiquettes d'admission de sécurité des pods Kubernetes intégrées à l' `trident`espace de noms pour l'exempter des contraintes de profil restreint :
