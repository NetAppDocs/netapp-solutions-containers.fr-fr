...
[%collapsible]
====
Cet exemple traite de la protection des données pour l'application PostgreSQL d'exemple, qui est installée dans l' `postgres`espace de noms. Pour plus de détails sur l'installation, voir link:deploy-vks-workloads.html["Déployez des charges de travail VKS à l'aide du stockage NetApp"].
