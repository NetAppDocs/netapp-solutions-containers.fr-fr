...
summary: 'Cette section est dédiée à la création et à la configuration d"un registre d"images privé soutenu par un stockage persistant fourni par Trident.' 
---
...
[role="lead"]
Pour la plupart des déploiements de Red Hat OpenShift, l'utilisation d'un registre public comme link:https://quay.io["Quay.io"^] ou link:https://hub.docker.com["DockerHub"^] répond aux besoins de la plupart des clients. Cependant, il arrive qu'un client souhaite héberger ses propres images privées ou personnalisées.
...
----
. Modifiez l'opérateur imageregistry en saisissant les paramètres de stockage suivants dans le `spec` section.
+
[listing]
----
...
NOTE: La configuration d'itinéraire ci-dessus est utilisée lorsque vous souhaitez un nom d'hôte personnalisé pour votre itinéraire.  Si vous souhaitez qu'OpenShift crée une route avec un nom d'hôte par défaut, vous pouvez ajouter les paramètres suivants au `spec` section: `defaultRoute: true` .
...
----
.. Pour référencer le secret d'extraction dans la définition du pod, ajoutez le paramètre suivant au `spec` section.
+
[listing]
----
imagePullSecrets:
