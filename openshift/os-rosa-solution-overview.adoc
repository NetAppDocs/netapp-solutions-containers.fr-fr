...
* link:https://portal.aws.amazon.com/billing/signup["compte AWS"^]
...
Utilisez l'option node-prep et installez Trident comme indiqué. Avant d'exécuter la commande d'installation, vous devez avoir téléchargé le package d'installation. Consultez link:https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-tridentctl.html#step-1-download-the-trident-installer-package["Téléchargez le package d'installation de Trident"^].
...
La configuration du back-end Trident indique à Trident comment communiquer avec le système de stockage (dans ce cas, FSx for ONTAP). Pour créer le back-end, nous fournirons les identifiants de la machine virtuelle de stockage à laquelle se connecter, ainsi que les interfaces de gestion du cluster et de données NFS. Nous utiliserons le link:https://docs.netapp.com/us-en/trident/trident-use/ontap-nas.html["pilote ontap-nas"^] pour provisionner des volumes de stockage dans le système de fichiers FSx.
...
----
Vous pouvez utiliser les commandes suivantes pour créer un PVC et vérifier qu'il a bien été créé.
...
image::redhat-openshift-container-rosa-017.png[table, schéma, lignes PostgreSQL]
