...
* *Mode couche 2.* Dans ce mode, un nœud du OpenShift cluster prend en charge le service et répond aux requêtes ARP pour cette adresse IP afin de la rendre accessible depuis l'extérieur du OpenShift cluster. Étant donné que seul ce nœud annonce l'adresse IP, il présente un goulot d'étranglement en termes de bande passante et des limitations de basculement lentes. Pour plus d'informations, consultez link:https://metallb.universe.tf/concepts/layer2/["Documentation du mode couche 2 de MetalLB"^].
* *Mode BGP.* Dans ce mode, tous les nœuds du cluster OpenShift établissent des sessions de peering BGP avec un routeur et annoncent les routes pour acheminer le trafic vers les adresses IP des services. La condition préalable est d’intégrer MetalLB avec un routeur dans ce réseau. En raison du mécanisme de hachage dans BGP, il présente certaines limitations lorsque la correspondance IP-vers-nœud pour un service change. Pour plus d’informations, consultez link:https://metallb.universe.tf/concepts/bgp/["Documentation du mode BGP de MetalLB"^].
...
NOTE: Si vous souhaitez configurer MetalLB en mode BGP, ignorez l’étape 6 ci-dessus et suivez la procédure dans link:https://metallb.universe.tf/concepts/bgp/["Documentation du mode BGP de MetalLB"^].
