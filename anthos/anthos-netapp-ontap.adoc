...
image:a-w-n-ontap-onpremises-virt-cloud.png["Figure montrant une boîte de dialogue d'entrée/sortie ou représentant un contenu écrit"]
...
Les deux systèmes sont alimentés par le logiciel de gestion de données NetApp ONTAP , le logiciel de gestion de données le plus avancé du secteur pour une gestion du stockage simplifiée, intégrée au cloud et hautement disponible afin de fournir la vitesse, l'efficacité et la sécurité de niveau entreprise dont votre infrastructure de données a besoin.

Pour plus d'informations sur les plateformes NetApp AFF et FAS, consultez le link:https://docs.netapp.com/platstor/index.jsp["NetApp documentation de la plateforme matérielle"^].
